...
export default function CreateServiceTeamForm() {
	...
	const handleCancel = () => {
		window.location.href = "/business-structure/display-service-team";
	};

	const validateField = (name, value) => {
		switch (name) {
			case "serviceTeamCode":
				if (!value) return "Service Team Code is required";
				if (!/^[a-zA-Z0-9]{4}$/.test(value)) {
					return "Service Team Code must be exactly 4 alphanumeric characters";
				}
				break;

			case "serviceTeamName":
				if (value.length > 30 || !/^[a-zA-Z0-9 ]+$/.test(value)) {
					return "Service Team Name must be alphanumeric and up to 30 characters";
				}
				break;
			default:
				return "";
	...
	const handleSubmit = async (e) => {
		...
		try {
			// Check if serviceTeamCode already exists
			const checkResponse = await fetch(
				`${BASE_URL_MS}/service-teams/${formData.serviceTeamCode}`
			);
			...
			if (checkResponse.status === 200) {
				...
				if (exists) {
					// setErrors((prev) => ({
					//   ...prev,
					//   serviceTeamCode: `Service Team Code "${formData.serviceTeamCode}" already exists. Please choose a new one.`,
					// }));
					toast.info(
						`Service Team Code "${formData.serviceTeamCode}" already exists. Please choose a new one.`
					);
					setIsSubmitting(false);
					return;
				}
				...
				throw new Error(
					"Failed to check Service Team Code availability"
				);
			}

			// Proceed with creation
			const response = await fetch(`${BASE_URL_MS}/service-teams`, {
				method: "POST",
				headers: {
			...
			if (!response.ok) {
				...
				throw new Error(
					errorData.message || "Failed to create service team"
				);
			}

			toast.success("Service Team created successfully!");
			...
			toast.error(
				error.message ||
					"An error occurred while creating the service team"
			);
		} finally {
			setIsSubmitting(false);
		}
	...
	const handleSaveManager = async () => {
		...
		const payload = {
			// serviceTeamCode: formData.serviceTeamCode, // ensure this is set
		...
		try {
			const resp = await fetch(`${BASE_URL_MS}/service-team-managers`, {
				method: "POST",
	...
	const handleSaveTableEmployee = async () => {
		...
		try {
			const resp = await fetch(`${BASE_URL_MS}/service-team-employees`, {
				method: "POST",
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify(payload),
			});
	...
	const fetchManagers = async () => {
		try {
			const response = await fetch(
				`${BASE_URL_MS}/service-team-managers`
			);
			if (!response.ok) {
	...
	const fetchTableEmployees = async () => {
		try {
			const response = await fetch(
				`${BASE_URL_MS}/service-team-employees`
			);
			if (!response.ok) {
				toast.error("Unable to fetch employees");
				return;
	...
	return (
		<>
			<div className="create-service-team-container">
				<form onSubmit={handleSubmit}>
					{/* Service Team Details */}
					<div className="create-service-team-header-box">
						<h2>Service Team Information</h2>
						<div className="data-container">
							<div className="data">
								<label htmlFor="serviceTeamCode">
									Service Team Code*
								</label>
								<span className="info-icon-tooltip">
									<i className="fas fa-info-circle" />
									<span className="tooltip-text">
										1- Service Team Code must be exactly 4
										digits. <br />
										2- Service Team Code must be unique.{" "}
										<br />
										3- Service Team Code must not contain
										any special characters. <br />
										4- Service Team Code must not contain
										any spaces. <br />
										5- Service Team Code once created then
								...
								<input
									type="text"
									id="serviceTeamCode"
									name="serviceTeamCode"
									value={formData.serviceTeamCode}
									onChange={(e) => {
									...
									placeholder="Enter Service Team Code"
									maxLength={4}
									required
								/>
								{errors.serviceTeamCode && (
							...
							<div className="data">
								<label htmlFor="serviceTeamName">
									Service Team Name*
								</label>
								<span className="info-icon-tooltip">
									<i className="fas fa-info-circle" />
									<span className="tooltip-text">
										Service Team Name must be alphanumeric
								...
								<input
									...
									value={formData.serviceTeamName}
									onChange={handleChange}
									placeholder="Enter Service Team Name"
									maxLength={30}
									required
								/>
								{errors.serviceTeamName && (
									<span className="error">
										{errors.serviceTeamName}
									</span>
								)}
					...
					<div className="create-service-team-button-group">
						<button
							type="submit"
							disabled={isSubmitting}
							className="create-service-team-save-button"
						>
							{isSubmitting ? "Saving..." : "Save"}
						</button>
						<button
							type="button"
							className="create-service-team-cancel-button"
