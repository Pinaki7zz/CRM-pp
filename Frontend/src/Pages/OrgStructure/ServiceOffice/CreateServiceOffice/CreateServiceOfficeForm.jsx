...
import "./CreateServiceOffice.css";

const BASE_URL_MS = import.meta.env.VITE_API_BASE_URL_MS;

export default function CreateServiceOfficeForm() {
	...
	const handleSubmit = async (e) => {
		...
		try {
			...
			const checkResponse = await fetch(
				`${BASE_URL_MS}/service-offices/${formData.serviceOfficeId}`
			);
			...
			if (checkResponse.status === 200) {
				...
				if (exists) {
					// setErrors(prev => ({
					//   ...prev,
					//   serviceOfficeId: `ID "${formData.serviceOfficeId}" already exists. Please use a different code.`,
					// }));
					alert(
						`ID "${formData.serviceOfficeId}" already exists. Please use a different code.`
					);
					setIsSubmitting(false);
					return;
				}
			...
			if (!response.ok) {
				...
				throw new Error(
					errorData.message || "Failed to create service office"
				);
			}

			alert("Service Office created successfully!");
			navigate("/business-structure/display-service-office");
		} catch (error) {
			console.error("Error creating service office:", error);
			alert(
				error.message ||
					"An error occurred while creating the service office"
			);
		} finally {
			setIsSubmitting(false);
		}
	...
	return (
		<div className="create-service-office-container">
			<form onSubmit={handleSubmit}>
				...
				<div className="create-service-office-header-box">
					<h2>Service Office Information</h2>
					<div className="data-container">
						<div className="data">
							<label htmlFor="serviceOfficeId">ID*</label>
							...
							<input
								...
								value={formData.serviceOfficeId}
								onChange={(e) => {
								...
								className={
									errors.serviceOfficeId ? "error" : ""
								}
							/>
							{errors.serviceOfficeId && (
						...
						<div className="data">
							<label htmlFor="serviceOfficeDesc">
								Description
							</label>
							<span className="info-icon-tooltip">
								<i className="fas fa-info-circle" />
								<span className="tooltip-text">
									Service Office Description must be
									alphanumeric and up to 50 characters
								</span>
							</span>
							<input
								type="text"
								id="serviceOfficeDesc"
								name="serviceOfficeDesc"
								value={formData.serviceOfficeDesc}
								onChange={handleChange}
								maxLength={50}
								placeholder="Enter service office description"
								className={
									errors.serviceOfficeDesc ? "error" : ""
								}
							/>
							{errors.serviceOfficeDesc && (
								<span className="error">
									{errors.serviceOfficeDesc}
								</span>
							)}
				...
				<div className="create-service-office-item-box">
				...
				<div className="create-service-office-button-group">
					<button
						type="submit"
						disabled={isSubmitting}
						className="create-service-office-save-button"
					>
						{isSubmitting ? "Saving..." : "Save"}
					</button>
					<button
						type="button"
						className="create-service-office-cancel-button"
