...
const serviceOfficeSchema = Joi.object({
	serviceOfficeId: Joi.string()
		.length(4)
		.pattern(/^[a-zA-Z0-9]+$/)
		.required()
		.messages({
	...
	serviceOfficeDesc: Joi.string()
		.max(50)
		.pattern(/^[a-zA-Z0-9 ]+$/)
		.messages({
			'string.base': 'Service Office Description should be a string',
			'string.max': 'Service Office Description should not exceed 50 characters',
			'string.pattern.base': 'Service Office Description should be alphanumeric',
		}),
...
const updateServiceOfficeSchema = Joi.object({
	organizationName: Joi.string()
		.max(30)
		.pattern(/^[a-zA-Z0-9 ]+$/),
	serviceOfficeDesc: Joi.string()
...
const validateServiceOffice = (data) => {
	return serviceOfficeSchema.validate(data, { abortEarly: false });
};

const validateServiceOfficeUpdate = (data) => {
	return updateServiceOfficeSchema.validate(data, { abortEarly: false });
};

module.exports = {
	validateServiceOffice,
	validateServiceOfficeUpdate
};