...
export default function CreateSalesTeamForm() {
	...
	const handleCancel = () => {
		window.location.href = "/business-structure/display-sales-team";
	};

	const validateField = (name, value) => {
		switch (name) {
			case "salesTeamCode":
				if (!value) return "Sales Team Code is required";
	...
	const handleSubmit = async (e) => {
		...
		try {
			// Check if salesTeamCode already exists
			const checkResponse = await fetch(
				`${BASE_URL_MS}/sales-teams/${formData.salesTeamCode}`
			);
			...
			if (checkResponse.status === 200) {
				...
				if (exists) {
					// setErrors((prev) => ({
					//   ...prev,
					//   salesTeamCode: `Sales Team Code "${formData.salesTeamCode}" already exists. Please choose a new one.`,
					// }));
					toast.info(
						`Sales Team Code "${formData.salesTeamCode}" already exists. Please choose a new one.`
					);
					setIsSubmitting(false);
					return;
				}
			} else if (checkResponse.status !== 404) {
				throw new Error("Failed to check Sales Team Code availability");
			}
			...
			if (!response.ok) {
				...
				throw new Error(
					errorData.message || "Failed to create sales team"
				);
			}
			...
			toast.error(
				error.message ||
					"An error occurred while creating the sales team"
			);
		} finally {
			setIsSubmitting(false);
		}
	...
	const handleSaveManager = async () => {
		...
		const payload = {
			// salesTeamCode: formData.salesTeamCode, // ensure this is set
		...
		try {
			const resp = await fetch(`${BASE_URL_MS}/sales-team-managers`, {
				method: "POST",
	...
	const handleSaveTableEmployee = async () => {
		...
		try {
			const resp = await fetch(`${BASE_URL_MS}/sales-team-employees`, {
				method: "POST",
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify(payload),
			});
	...
	const fetchManagers = async () => {
		try {
			const response = await fetch(`${BASE_URL_MS}/sales-team-managers`);
			if (!response.ok) {
	...
	const fetchTableEmployees = async () => {
		try {
			const response = await fetch(`${BASE_URL_MS}/sales-team-employees`);
			if (!response.ok) {
				toast.error("Unable to fetch employees");
				return;
			}
	...
	return (
		<>
			<div className="create-sales-team-container">
				<form onSubmit={handleSubmit}>
					{/* Sales Team Details */}
					<div className="create-sales-team-header-box">
						<h2>Sales Team Information</h2>
						<div className="data-container">
							<div className="data">
								<label htmlFor="salesTeamCode">
									Sales Team Code*
								</label>
								<span className="info-icon-tooltip">
									<i className="fas fa-info-circle" />
									<span className="tooltip-text">
										1- Sales Team Code must be exactly 4
										digits. <br />
										2- Sales Team Code must be unique.{" "}
										<br />
										3- Sales Team Code must not contain any
										special characters. <br />
										4- Sales Team Code must not contain any
										spaces. <br />
										5- Sales Team Code once created then it
										can be not delete. <br />
									</span>
								</span>
								<input
									type="text"
									...
									placeholder="Enter Sales Team Code"
									maxLength={4}
									required
								/>
								{errors.salesTeamCode && (
							...
							<div className="data">
								<label htmlFor="salesTeamName">
									Sales Team Name*
								</label>
								<span className="info-icon-tooltip">
									<i className="fas fa-info-circle" />
									<span className="tooltip-text">
										Sales Team Name must be alphanumeric and
										up to 30 characters.
									</span>
								</span>
								<input
									...
									placeholder="Enter Sales Team Name"
									maxLength={30}
									required
								/>
					...
					<div className="create-sales-team-button-group">
						<button
							type="submit"
							disabled={isSubmitting}
							className="create-sales-team-save-button"
						>
							{isSubmitting ? "Saving..." : "Save"}
						</button>
						<button
							type="button"
							className="create-sales-team-cancel-button"
